...
const removeVariableDeclarator = require('../utils/remove-variable-declarator')
...
module.exports = (file, api) => {
  ...
  const transformBodyElementToProperty = bodyElement => {
    if (isConstructor(bodyElement) && bodyElement.body.body.length > 1) {
      ...
      const propName = bodyElement.expression.left.property.name || bodyElement.expression.left.property.value
      try {
        return j.property(
          'init',
